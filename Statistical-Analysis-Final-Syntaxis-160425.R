...
# PART A: Models reporting Average Marginal Effects and Robust Standard Errors (as it was presented in the master thesis report)
...
#### 3.3) Logistic Model 3: Outcome = Frames / Predictor = Female / Controls = Government + newspapers
# Logit CRIME Model 3
...
# PART B: Models reporting log-odds and Robust Standard Errors (as it was presented in the master thesis report)

#### 3.5) Model 4 - log-odds + RSE (Outcome = Frame / Preditor = Year / Control = newspaper)
# Logistic regression CRIME model 4
logit4_crime <- glm(crime_frame_dummy ~ Year_category + newspaper_name, data = data, family = binomial)
logit4_crime_RSE <- coeftest(logit4_crime, vcov. = vcovHC(logit4_crime, type = "HC0"))

# Logistic regression MIGRATION model 4
logit4_migration <- glm(migration_frame_dummy~ Year_category + newspaper_name, data = data, family = binomial)
logit4_migration_RSE <- coeftest(logit4_migration, vcov. = vcovHC(logit4_migration, type = "HC0")) # With robust standrd errors

# Logistic regression SEXVI model 4
logit4_sexvi <- glm(sexvi_frame_dummy~ Year_category + newspaper_name, data = data, family = binomial)
logit4_sexvi_RSE <- coeftest(logit4_sexvi, vcov. = vcovHC(logit4_sexvi, type = "HC0")) 

#### 3.6) Model 5 - log-odds + RSE (Outcome = Frame / Preditor = Government / Control = newspaper)
# Logistic regression CRIME model 5
logit5_crime <- glm(crime_frame_dummy ~ Government + newspaper_name, data = data, family = binomial)
logit5_crime_RSE <- coeftest(logit5_crime, vcov. = vcovHC(logit5_crime, type = "HC0"))

# Logistic regression MIGRATION model 5
logit5_migration <- glm(migration_frame_dummy~ Government + newspaper_name, data = data, family = binomial)
logit5_migration_RSE <- coeftest(logit5_migration, vcov. = vcovHC(logit5_migration, type = "HC0"))

# Logistic regression SEXVI model 5
logit5_sexvi <- glm(sexvi_frame_dummy~ Government + newspaper_name, data = data, family = binomial)
logit5_sexvi_RSE <- coeftest(logit5_sexvi, vcov. = vcovHC(logit5_sexvi, type = "HC0"))

#### 3.7) Model 6 - log-odds + RSE (Outcome = Frame / Preditor = Female /Control = Government +newspaper)
# Logistic regression CRIME model 6
logit6_crime <- glm(crime_frame_dummy ~ Female + Government + newspaper_name, data = data, family = binomial)
logit6_crime_RSE <- coeftest(logit6_crime, vcov. = vcovHC(logit6_crime, type = "HC0"))

# Logistic regression MIGRATION model 6
logit6_migration <- glm(migration_frame_dummy~ Female+ Government + newspaper_name, data = data, family = binomial)
logit6_migration_RSE <- coeftest(logit6_migration, vcov. = vcovHC(logit6_migration, type = "HC0"))

# Logistic regression SEXVI model 6
logit6_sexvi <- glm(sexvi_frame_dummy~ Female + Government + newspaper_name, data = data, family = binomial)
logit6_sexvi_RSE <- coeftest(logit6_sexvi, vcov. = vcovHC(logit6_sexvi, type = "HC0"))

#### 3.8) Reporting the models with LOG-ODDS and RSE
ordered_names_logodds <- c("Intercept",
  "2018" , "2019", "2020",
  "2021", "2022", "2023",
  "Ojo", "Peru21", "Trome",
  "Vizcarra", "Sagasti", "Castillo", "Boluarte"
...
# Table 5: Crime frames
screenreg(list(logit4_crime_RSE,logit5_crime_RSE),
          custom.model.names = c("Crime", "Crime"),
          stars = c(0.05, 0.01, 0.001), custom.header = list("Model 1" = 1, "Model 2" = 2),
          custom.coef.names = ordered_names_logodds, 
          reorder.coef=c(2,3,4,5,6,7,11,12,13,14,8,9,10,1), 
          groups = list("Year (ref = 2017)" = 1:6, "Government (ref = Kuczynski)" = 7:10,
                        "Newspaper (ref = Correo)" = 11:13))

# Table 6: Migration frames
screenreg(list(logit4_migration_RSE,logit5_migration_RSE),
          custom.model.names = c("Migration", "Migration"),
          stars = c(0.05, 0.01, 0.001), custom.header = list("Model 1" = 1, "Model 2" = 2),
          custom.coef.names = ordered_names_logodds, 
          reorder.coef=c(2,3,4,5,6,7,11,12,13,14,8,9,10,1), 
...
# Table 7: Sexual violence frames
screenreg(list(logit4_sexvi_RSE,logit5_sexvi_RSE),
          custom.model.names = c("Sexual Violence", "Sexual Violence"),
          stars = c(0.05, 0.01, 0.001), custom.header = list("Model 1" = 1, "Model 2" = 2),
          custom.coef.names = ordered_names_logodds, 
          reorder.coef=c(2,3,4,5,6,7,11,12,13,14,8,9,10,1), 
...
# Table 8: Gender
screenreg(list(logit6_crime_RSE,logit6_migration_RSE,logit6_sexvi_RSE),
          custom.model.names = c("Crime", "Migration", "Sexual Violence"),
          stars = c(0.05, 0.01, 0.001), custom.header = list("Model 1" = 1, "Model 2" = 2, "Model 3" = 3),
          custom.coef.names = ordered_names_2_logodds, 
          groups = list("Government (ref = Kuczynski) " = 2:5, "Newspaper (ref = Correo)" = 6:8),
